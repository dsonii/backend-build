...
bookingSchema.statics = {
  async transformSingleData(item) {
    return {
      ...
      created_at: moment(item.createdAt).tz(DEFAULT_TIMEZONE).format(`${DEFAULT_DATEFORMAT} ${DEFAULT_TIMEFORMAT}`),
    };
  },
  async transformData(data) {
    ...
    for (let item of data) {
      ...
      selectableItems.push({
        ...
        discount: item.discount ? item.discount : '',
        date_time: `${moment
          .utc(item.booking_date).tz(DEFAULT_TIMEZONE).format('YYYY-MM-DD')} <br/> <small>${item.start_time} - ${item.drop_time}</small>`,
        tax: item.tax,
        tax_amount: item.tax_amount,
        fee: item.fee,
        final_total_fare: item.final_total_fare,
